...
def consultar_all():
    ...
    for inst in instituciones:
        print(f"   ID: {inst.id} | {inst.nombre} | {inst.ciudad}, {inst.pais}")
    ...
    for dept in departamentos:
        print(f"   ID: {dept.id} | Código: {dept.codigo} | {dept.nombre}")
    ...
    for inv in investigadores:
        print(f"   ID: {inv.id} | {inv.nombre} {inv.apellido} | Área: {inv.area_investigacion}")
    ...
    for pub in publicaciones:
        print(f"   ID: {pub.id} | {pub.titulo} | Tipo: {pub.tipo_publicacion} | Fecha: {pub.fecha_publicacion}")
